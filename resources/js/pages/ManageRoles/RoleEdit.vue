...
function confirmSubmit() {
  ...
  form.put(`/roles/${props.role.id}`, {
    preserveScroll: true,
    onSuccess: () => {
      success(`Role "${form.name}" berhasil diupdate.`)
      closeSubmitModal(true)
    },
    onError: (errs: any) => {
      const msg = errs?.error || "Gagal update role. Cek input."
      error(String(msg))
    },
    onFinish: () => {
      loading.value = false
  ...
  <AppLayout title="Manage Roles" currentPage="roles.index">
    <div class="space-y-6">
      <div class="bg-white rounded-lg shadow p-6 border border-gray-200">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Edit Role</h2>
        ...
        <form @submit.prevent="openSubmitModal" class="space-y-6">
          ...
          <div>
            ...
            <input
              v-model="form.name"
              type="text"
              placeholder="e.g., Planning Manager"
    ...
    <ConfirmDialog
      ...
      title="Update Role?"
      message="Pastikan perubahan role dan permission sudah benar. Setelah disimpan, perubahan akan berlaku di sistem."
