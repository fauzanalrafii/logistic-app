...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('vl_bep_projections', function (Blueprint $table) {
        ...
        // Pindahkan data status string → status_id
        DB::table('vl_bep_projections')->get()->each(function ($row) {
            $status = DB::table('vl_status')
                ->where('type', 'bep')
                ->where('name', $row->status)
                ->first();

            DB::table('vl_bep_projections')
                ->where('id', $row->id)
